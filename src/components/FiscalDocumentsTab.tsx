...
export function FiscalDocumentsTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error((err as Error).message || t("Failed to load documents."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const download = (r: ReceiptDocument) => {
    ...
    requireStepUp(async (token) => {
      ...
      try {
        ...
        toast.error((err as Error).message || t("Could not download the receipt."));
      } finally {
        setBusyKey(null);
      }
    });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
            {t("Tax documents & receipts")}
          </p>
          <p className="text-[11px] text-muted-foreground">
            {t("Download payment receipts. Official NF-e/NFS-e coming soon.")}
          </p>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-mono text-[10px] uppercase tracking-widest text-warning">
              {t("Official fiscal issuance")}
            </p>
            <p className="mt-1 text-[11px] text-muted-foreground">
              {fiscal?.message_en ??
                t("Official NF-e/NFS-e issuance will be available once EnergyPay's tax profile and digital certificate are configured.")}
            </p>
            {fiscal && (
              <div className="mt-2 flex flex-wrap gap-1.5">
                <Badge variant="outline" className="font-mono text-[8px] uppercase tracking-widest text-muted-foreground">
                  {t("Tax profile:")} {fiscal.company_profile_status}
                </Badge>
                <Badge variant="outline" className="font-mono text-[8px] uppercase tracking-widest text-muted-foreground">
                  {t("Certificate:")} {fiscal.certificate_status}
                </Badge>
                <Badge variant="outline" className="font-mono text-[8px] uppercase tracking-widest text-muted-foreground">
                  {fiscal.can_issue_official ? t("Ready to issue") : t("Issuance pending")}
                </Badge>
              </div>
            )}
          </div>
      ...
      {loading ? (
        ...
        <Card className="border-dashed border-border bg-card/40 p-10 text-center">
          ...
          <p className="font-mono text-[11px] text-muted-foreground">{t("No receipts available.")}</p>
          <p className="mt-1 text-[10px] text-muted-foreground/60">
            {t("Each paid charge generates a receipt for download here.")}
          </p>
        </Card>
      ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-mono text-sm font-semibold">{t("Payment receipt")}</p>
                      <Badge variant="outline" className="font-mono text-[8px] uppercase tracking-widest text-muted-foreground">
                        {r.source === "crypto_invoice" ? t("Crypto") : t("Subscription")}
                      </Badge>
                    </div>
      ...
      <div className="flex items-start gap-2 rounded-md border border-border bg-card/40 p-3">
        ...
        <p className="font-mono text-[9px] leading-relaxed text-muted-foreground">
          {t("Estes documentos são recibos de pagamento gerados pela EnergyPay. Eles não substituem uma Nota Fiscal eletrônica (NF-e/NFS-e) oficial. · This document is a payment receipt generated by EnergyPay. It is not an official Brazilian NF-e/NFS-e fiscal invoice.")}
        </p>
      </div>
