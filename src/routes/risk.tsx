...
export const Route = createFileRoute("/risk")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Counterparty risk, credit exposure, collateral ratios and settlement guarantees.",
      },
    ],
  }),
  component: RiskPage,
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-end justify-between">
        ...
        <SeverityBadge
          level={restricted > 0 ? "WARN" : "OK"}
          label={`${monitored} MONITOR · ${restricted} RESTRICTED`}
        />
      </div>
      ...
      <KpiStrip>
        ...
        <KpiTile
          ...
          value={`${((totalColl / totalExp) * 100).toFixed(1)}%`}
          tone="ok"
        />
        <KpiTile
          label="Settlement Guarantee"
          value={fmtBRLm(420_000_000)}
          unit="BRL"
          tone="primary"
          sub="multi-tier waterfall"
        />
        <KpiTile label="Default Pool Util." value="11%" tone="ok" sub="T2 unactivated" />
        <KpiTile
          label="VaR · 95% · 1d"
          value={fmtBRLm(12_840_000)}
          tone="warn"
          sub="parametric · normal"
        />
      </KpiStrip>

      <Panel
        title="Counterparty Risk Register"
        subtitle="Exposure · collateral · default probability · settlement confidence"
      >
              ...
              {COUNTERPARTIES.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="text-right">
                    <CellNum>{fmtBRLm(c.exposureBRL)}</CellNum>
                  </td>
                  <td className="text-right">
                    <CellNum>{fmtBRLm(c.collateralBRL)}</CellNum>
                  </td>
                  <td className="text-right">
                    <CellNum
                      tone={c.collateralRatio < 1 ? "bad" : c.collateralRatio < 1.1 ? "warn" : "ok"}
                    >
                      {c.collateralRatio.toFixed(2)}×
                    </CellNum>
                  </td>
                  <td className="text-right">
                    <CellNum tone={c.defaultProbBps > 80 ? "warn" : "default"}>
                      {c.defaultProbBps}
                    </CellNum>
                  </td>
                  <td className="text-right">
                    <CellNum tone={c.settlementConfidence < 75 ? "warn" : "ok"}>
                      {fmtPct(c.settlementConfidence, 0)}
                    </CellNum>
                  </td>
                  <td className="text-right">
                    <CellNum>{c.openContracts}</CellNum>
                  </td>
                  <td>
                    <span className="inline-flex items-center gap-1.5">
                      <StatusDot
                        tone={
                          c.status === "ACTIVE" ? "ok" : c.status === "MONITOR" ? "warn" : "bad"
                        }
                      />
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-3">
        <Panel
          ...
          subtitle="Counterparty × tenor bucket · BRL"
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  ...
                  {TENORS.map((t) => (
                    <th key={t} className="label-op p-2 text-right">
                      {t}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {COUNTERPARTIES.slice(0, 10).map((c, i) => {
                  ...
                  const buckets = TENORS.map((_, j) =>
                    Math.round(total * (0.34 - j * 0.06 + ((i + j) % 3) * 0.02)),
                  );
                  const max = Math.max(...buckets);
                  return (
                    <tr key={c.id}>
                      <td className="border-b border-border/60 px-2 py-1.5 text-[11.5px]">
                        {c.shortName}
                      </td>
                      {buckets.map((b, j) => {
                          ...
                          <td key={j} className="border-b border-border/60 px-1 py-1.5 text-right">
                            <div
                              ...
                              style={{
                                ...
                                color:
                                  intensity > 0.55 ? "oklch(0.14 0.02 250)" : "var(--foreground)",
                              }}
                            >
                              {fmtBRLm(b)}
                            </div>
        ...
        <Panel
          title="Default Waterfall Simulation"
          subtitle="Counterparty stress · cascade absorption"
        >
          <div className="space-y-2">
            ...
            <div className="mt-3 rounded-sm border border-warning/40 bg-warning/5 p-2 text-[11px]">
              <p className="font-mono text-[9.5px] uppercase tracking-widest text-warning">
                Scenario · Comerc default
              </p>
              <p className="mt-1 text-muted-foreground leading-snug">
                Loss absorbed at tier 3 — no mutualized exposure required. System remains within
                tolerance.
              </p>
            </div>
          </div>
