  ...
  head: () => ({
    meta: [
      { title: "Treasury & Settlement Rails — EnergyPay" },
      {
        name: "description",
        content:
          "Stellar settlement rail monitoring, custody operations and BRL settlement architecture.",
      },
    ],
  }),
    ...
    <div className="space-y-3">
      ...
      <KpiStrip>
        <KpiTile
          ...
          unit="XLM"
          tone="primary"
          sub={pk ? shortHash(pk) : "no operator"}
        />
        <KpiTile
          label="EPWR Inventory"
          value={Number(eprw).toLocaleString("en-US", { maximumFractionDigits: 2 })}
          unit="EPWR"
          tone="ok"
        />
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-3">
        <div className="space-y-3 xl:col-span-2">
          ...
          <Panel
            ...
            subtitle="Generator → Clearing → Counterparty · programmable leg"
          >
            ...
            <tbody>
              {activity.events.slice(0, 8).map((ev) => (
                <tr key={ev.id}>
                  ...
                  <td className="text-right">
                    <CellNum>{ev.amount ?? "—"}</CellNum>
                  </td>
                  <td className="font-mono text-[10.5px]">{ev.asset ?? "—"}</td>
                  <td>
                    <SeverityBadge
                      level={ev.successful ? "OK" : "CRITICAL"}
                      label={ev.successful ? "FINALIZED" : "FAILED"}
                    />
                  </td>
                  <td className="text-right">
                    <a
              ...
              {activity.events.length === 0 && (
                <tr>
                  <td
                    colSpan={6}
                    className="text-center font-mono text-[10.5px] uppercase tracking-widest text-muted-foreground"
                  >
                    No ledger activity in window
                  </td>
                </tr>
              )}
            </tbody>
