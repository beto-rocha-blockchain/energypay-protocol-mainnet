  ...
  head: () => ({
    meta: [
      { title: "Clearing House Console — EnergyPay" },
      {
        name: "description",
        content:
          "Bilateral contract lifecycle, margin monitoring, settlement state machine and ledger anchoring.",
      },
    ],
  }),
...
const QUEUE_SEED: Array<{
...
}> = [
  {
    ...
    notional: 511_520,
    state: "FINALIZED",
    submitted: new Date(Date.now() - 8 * 60_000).toISOString(),
    finality: 4120,
    txHash: "9f3ab27c1d8e0a4f51b",
  },
  {
    id: "CLR-77822",
    cp: "Cemig D MG",
    ...
    notional: 242_880,
    state: "BROADCAST",
    submitted: new Date(Date.now() - 3 * 60_000).toISOString(),
    finality: 0,
    txHash: "771dffaa20419c8b00a",
  },
  {
    id: "CLR-77823",
    cp: "Engie BR",
    ...
    notional: 698_400,
    state: "SIGNED",
    submitted: new Date(Date.now() - 90_000).toISOString(),
  },
  {
    id: "CLR-77824",
    cp: "Equatorial NE",
    ...
    notional: 207_400,
    state: "VALIDATED",
    submitted: new Date(Date.now() - 40_000).toISOString(),
  },
  {
    id: "CLR-77825",
    cp: "Klabin",
    ...
    notional: 315_000,
    state: "INTAKE",
    submitted: new Date(Date.now() - 12_000).toISOString(),
  },
  {
    id: "CLR-77820",
    cp: "Comerc",
    ...
    notional: 445_725,
    state: "REJECTED",
    submitted: new Date(Date.now() - 11 * 60_000).toISOString(),
  },
];
...
function ClearingPage() {
  ...
  const counts = useMemo(() => {
    const c: Record<ClrState, number> = {
      INTAKE: 0,
      VALIDATED: 0,
      SIGNED: 0,
      BROADCAST: 0,
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-end justify-between">
        ...
        <SeverityBadge
          level="OK"
          label={`SESSION CLR-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}`}
        />
      </div>
      ...
      <KpiStrip>
        <KpiTile
          label="In Queue"
          value={fmtNum(
            queue.filter((q) => q.state !== "FINALIZED" && q.state !== "REJECTED").length,
          )}
          tone="primary"
        ...
        <KpiTile
          label="Avg Finality"
          value={`${(avgFinality / 1000).toFixed(2)}s`}
          tone={avgFinality < 6000 ? "ok" : "warn"}
        />
        <KpiTile
          label="Margin Calls · pending"
          value="3"
          tone="warn"
          sub="2 monitored · 1 escalated"
        />
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-5">
        <Panel
          ...
          subtitle="Lifecycle · throughput per stage"
            ...
            {STATES.map((s, i) => (
              <div key={s} className="rounded-sm border border-border bg-background/40 p-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <StatusDot
                    tone={s === "FINALIZED" ? "ok" : s === "BROADCAST" ? "info" : "muted"}
                  />
                </div>
                ...
                <div className="mt-1 h-0.5 w-full bg-muted overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(counts[s] / Math.max(1, queue.length)) * 100}%` }}
                  />
                </div>
                {i < STATES.length - 1 && (
                ...
                {queue.map((q) => (
                  <tr key={q.id}>
                    ...
                    <td className="text-right">
                      <CellNum>{fmtNum(q.volMwh)} MWh</CellNum>
                    </td>
                    <td className="text-right">
                      <CellNum>R$ {q.priceMwh}</CellNum>
                    </td>
                    <td className="text-right">
                      <CellNum>{fmtBRLm(q.notional)}</CellNum>
                    </td>
                    <td>
                      <SeverityBadge
                        level={
                    ...
                    <td className="font-mono text-[10.5px]">
                      {q.txHash ? (
                        <span className="text-primary">{shortHash(q.txHash)}</span>
                      ) : (
        ...
        <Panel
          ...
          subtitle="Live counterparty margin envelope"
        ...
        >
          <div className="space-y-2">
            {COUNTERPARTIES.slice(0, 8).map((c) => {
              ...
              const tone =
                c.collateralRatio < 1.0 ? "bad" : c.collateralRatio < 1.1 ? "warn" : "ok";
              return (
                <div
                  key={c.id}
                ...
                >
                  ...
                  <div className="mt-1 h-1 w-full bg-muted overflow-hidden">
                    <div
                      className={`h-full ${tone === "bad" ? "bg-destructive" : tone === "warn" ? "bg-warning" : "bg-success"}`}
                      style={{ width: `${(pct / 160) * 100}%` }}
                    />
                  </div>
      ...
      <Panel
        title="Audit Checkpoint Timeline"
        subtitle="Immutable lifecycle anchors · last 90 minutes"
      >
          ...
          {[
            {
              ...
              icon: Circle,
              tone: "info" as const,
              meta: "operator · supervisor.dias",
            },
            {
              ts: 4,
              label: "Margin re-evaluation",
              icon: CheckCircle2,
              tone: "ok" as const,
              meta: "Δ ratio reviewed for 16 CPs",
            },
            {
              ts: 11,
              label: "Contract intake batch #41",
              icon: CheckCircle2,
              tone: "ok" as const,
              meta: "9 tickets · 23 ledger ops",
            },
            {
              ts: 19,
              label: "Comerc margin call",
              icon: AlertTriangle,
              tone: "warn" as const,
              meta: "0.97× ratio · 1.05× target",
            },
            {
              ts: 26,
              label: "Ledger anchor finalized",
              icon: CheckCircle2,
              tone: "ok" as const,
              meta: "block 51 224 197 · 4 anchors",
            },
            {
              ts: 38,
              label: "Oracle override · PLD NE",
              icon: AlertTriangle,
              tone: "warn" as const,
              meta: "fallback B engaged",
            },
            {
              ts: 51,
              label: "Settlement window closed",
              icon: CheckCircle2,
              tone: "ok" as const,
              meta: "$8.4M cleared",
            },
            ...
            <li key={i} className="relative flex gap-3 border-l border-border py-2 pl-4">
              <span
                className={`absolute -left-[5px] top-3 inline-block h-2.5 w-2.5 rounded-full ${e.tone === "ok" ? "bg-success" : e.tone === "warn" ? "bg-warning" : "bg-primary"}`}
              />
