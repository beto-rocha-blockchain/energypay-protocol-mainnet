  ...
  head: () => ({
    meta: [
      { title: "Market Operations Center — EnergyPay" },
      {
        name: "description",
        content:
          "Real-time settlement throughput, cleared notional, exposure and operational telemetry.",
      },
    ],
  }),
...
function MarketOpsPage() {
  ...
  const reconHealth = Math.round(
    100 -
      (RECON_EXCEPTIONS.filter((e) => e.state !== "RESOLVED").length / RECON_EXCEPTIONS.length) *
        100 +
      14,
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <KpiStrip>
        ...
        <KpiTile
          label="Open Exposure"
          value={fmtBRLm(exposure)}
          unit="BRL"
          tone="warn"
          sub="across 16 counterparties"
        />
        <KpiTile
          label="Collateral Coverage"
          value={`${((collateral / exposure) * 100).toFixed(1)}%`}
          tone="ok"
          sub={`${fmtBRLm(collateral)} pledged`}
        />
        <KpiTile
          label="Reconciliation Health"
          value={`${Math.min(99, reconHealth)}%`}
          tone={reconHealth > 92 ? "ok" : "warn"}
          sub={`${RECON_EXCEPTIONS.filter((e) => e.state === "OPEN").length} open exceptions`}
        />
        <KpiTile
          label="Finality p95"
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-3">
        <Panel
          ...
          subtitle="60-minute window · Δ tx/min · cleared notional"
          className="xl:col-span-2"
          right={<SeverityBadge level="OK" label="STREAMING" />}
        >
          <div className="h-56">
        ...
        <Panel
          ...
          subtitle="Severity-coded operational stream"
          right={
            <SeverityBadge
              level="WARN"
              label={`${OPS_ALERTS.filter((a) => a.severity !== "INFO").length} ACTIVE`}
            />
          }
        >
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-3">
        <Panel
          title="Counterparty Activity · 60m"
          subtitle="Cleared notional density per desk"
          className="xl:col-span-2"
        >
          <div className="h-44">
            <ResponsiveContainer>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => [`R$ ${v.toFixed(1)}M`, "exposure"]}
                />
        ...
        <Panel
          ...
          subtitle="Settlement guarantee pool · margin headroom"
        >
          <div className="space-y-3">
            {[
              ...
              <div key={b.name}>
                <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                  <span>{b.name}</span>
                  <span className={b.tone === "warn" ? "text-warning" : "text-success"}>
                    {b.util}%
                  </span>
                </div>
                <div className="mt-1 h-1.5 w-full overflow-hidden rounded-sm bg-muted">
                  <div
                    className={`h-full ${b.tone === "warn" ? "bg-warning" : "bg-success"}`}
                    style={{ width: `${b.util}%` }}
                  />
                </div>
              </div>
            ...
            <tbody>
              {COUNTERPARTIES.slice(0, 10).map((c) => (
                <tr key={c.id}>
                  ...
                  <td>
                    <SeverityBadge level="OK" label={c.rating} />
                  </td>
                  ...
                  <td className="text-right">
                    <CellNum tone={c.settlementConfidence < 70 ? "warn" : "ok"}>
