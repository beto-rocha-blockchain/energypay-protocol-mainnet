...
function generate(): ReconciliationRow[] {
  return RECON_EXCEPTIONS.map((x, i) => {
    const cp =
      COUNTERPARTIES.find((c) => c.shortName === x.counterparty) ??
    ...
    return {
      ...
      kind:
        x.kind === "PRICE_MISMATCH"
          ? "PRICE_MISMATCH"
          : x.kind === "VOLUME_DRIFT"
            ? "VOLUME_DRIFT"
            : x.kind === "ORACLE_DIVERGENCE"
              ? "ORACLE_DIVERGENCE"
              : x.kind === "LEDGER_GAP"
                ? "LEDGER_GAP"
                : "TIMESTAMP_SKEW",
      ...
      assignedTo:
        x.state === "INVESTIGATING"
          ? ["ops.cardoso", "ops.menezes", "compliance.silva"][i % 3]
          : undefined,
...
export const reconciliationService: ReadService<ReconciliationRow, void> = {
  async list() {
    ...
    return {
      ...
      asOf: nowIso(),
      source: "MOCK",
    } as ListResult<ReconciliationRow>;
  },
  async get(id) {
